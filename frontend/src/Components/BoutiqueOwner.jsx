...
const BoutiqueOwner = () => {
  ...
  const boutique = selectedBoutique; // puisque c'est l'objet complet
    ...
    <>
      {!showStat ? (
        <div className="max-w-[1400px] mx-auto p-4">
          {/* Conteneur principal avec image pleine largeur */}
          <div className="flex flex-col gap-4 rounded-xl shadow-xl bg-gray-100 p-4 w-full">
          ...
          {/* GALLERY - maintenant en dessous */}
          <div className="mt-6 w-full">
            {boutique.products?.length > 0 && (
              <div className="w-full grid grid-cols-1 lg:grid-cols-1 gap-8 px-2 sm:px-4">
                <Gallery boutique={boutique} products={boutique.products} />
              </div>
            )}
          </div>
        </div>
